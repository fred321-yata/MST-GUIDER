// Correlate the code's room grid against each sheet using corrected ink
// detection: ink = anything that is NOT bright gray paper.
...
for (const floor of [1, 2, 3, 4]) {
  ...
  for (let y = 0; y < SH; y++) for (let x = 0; x < SW; x++) {
    ...
    if (!(gray && lum > 160)) ink[y * SW + x] = 1
  }
  let inkCount = 0
  for (const v of ink) inkCount += v
  const px = (p) => Math.round((p / 100) * (SW - 1))
  const py = (p) => Math.round((p / 100) * (SH - 1))
  // Precompute grid line spans for a shift; score = fraction of line pixels on ink.
  const score = (dx, dy) => {
    let hit = 0, total = 0
    for (const vx of CODE_V) {
      const X = px(vx + dx)
      for (let t = py(GRID.y0 + dy); t <= py(GRID.y1 + dy); t += 1) {
        for (const w of [0, 1]) {
          total++
          if (ink[t * SW + X + w]) hit++
        }
    ...
    for (const hy of CODE_H) {
      const Y = py(hy + dy)
      for (let t = px(GRID.x0 + dx); t <= px(GRID.x1 + dx); t += 1) {
        for (const w of [0, 1]) {
          total++
          if (ink[(Y + w) * SW + t]) hit++
        }
  ...
  console.log(
    `floor ${floor}: ink ${(100 * inkCount / (SW * SH)).toFixed(1)}% | base ${base.toFixed(3)} → ` +
      `best dx=${best.dx.toFixed(1)} dy=${best.dy.toFixed(1)} score=${best.s.toFixed(3)}`,
  )
}
